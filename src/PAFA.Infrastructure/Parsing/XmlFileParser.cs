...
///
/// Structure 1 � �l�ments enfants comme colonnes (format "row-based") :
///   &lt;Report&gt;
///     &lt;Row&gt;
...
///   &lt;/Report&gt;
///
/// Dans les deux cas, le r�sultat produit des RawDataRow normalis�s
/// identiques � ceux produits par ExcelFileParser et CsvFileParser.
/// Le m�me MetricValueMapper et ImportValidationService s'appliquent sans modification.
/// </summary>
public sealed class XmlFileParser : IFileParser
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            // ?? D�tecter les �l�ments "ligne" ??????????????????????????????
            // On prend les premiers enfants directs de la racine qui ont
            // eux-m�mes des enfants ou des attributs � ce sont nos lignes.
            ...
            if (rowElements.Count == 0)
                return Task.FromResult(Fail(fileName,
                    "Aucune ligne de donn�es trouv�e dans le XML. " +
                    "V�rifiez la structure : �l�ments enfants ou attributs attendus."));
            ...
            if (rows.Count == 0)
                return Task.FromResult(Fail(fileName,
                    "Le fichier XML ne contient aucune ligne de donn�es valide."));
    ...
    /// <summary>
    /// Normalise un nom d'�l�ment/attribut XML vers le format attendu par
